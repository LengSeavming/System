...
export class PetDialogComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        public data: { title: string; pet: Data; setup: any },
        private dialogRef: MatDialogRef<PetDialogComponent>,
    ...
    ngOnInit(): void {
        ...
        this.data.pet != null
            ? (this.src = `${env.FILE_BASE_URL}${this.data.pet.image}`)
            : '';
    ...
    srcChange(base64: string): void {
        ...
        this.petForm.get('image').setValue(base64);
    }
    ...
    onFileChange(event: any): void {
        ...
        if (file && file.type.startsWith('image/')) {
            ...
            reader.onload = (e: any) => {
                ...
                this.petForm.get('image')?.setValue(e.target.result);
            };
    ...
    ngBuilderForm(): void {
        ...
        this.petForm = this.formBuilder.group({
            code: [this.data?.pet?.code || null, [Validators.required]],
            name: [this.data?.pet?.name || null, [Validators.required]],
            type_id: [this.data?.pet?.type?.id || null, [Validators.required]],
            image: [null, this.data.pet == null ? Validators.required : []],
            unit_price: [
                this.data?.pet?.unit_price || null,
                [Validators.required],
            ],
        });
    }
    ...
    submit() {
        ...
        this.data.pet == null ? this.create() : this.update();
    }
    ...
    update(): void {
        ...
        this.petService.update(this.data.pet.id, this.petForm.value).subscribe({
            next: (response) => {
                // Transform the response data into the format expected by the parent component
                const pet: Data = {
                    id: response.data.id,
